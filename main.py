...
# row = soup.find_all("div", class_="row")[2].find("div", class_="name")  # ПО Индексу и по цепочки
# row = soup.find_all("div", {"data-set": "salary"})  # в словарь берем тег с тире или пробелом(не допустимый символ в питоне)
# row = soup.find_all("div", string="Alena")  # доступ по содержимому
# row = soup.find("div", string="Alena").parent  # к родительскому див
# row = soup.find("div", string="Alena").parent.parent  # к родительскому родительскому див
# row = soup.find("div", string="Alena").find_parent(class_="row")  # к родительскому по классу
# row = soup.find("div", id="whois3")  # по id
# row = soup.find("div", id="whois3").find_next_sibling()  # по id к следующему по коду
# row = soup.find("div", id="whois3").find_previous_sibling()  # по id к предыдущему
...
#     res = re.search(pattern, s).group()  # то же самое, что и выше
#     print(res)
#
#
# row = soup.find_all("div", {"data-set": "salary"})
# for i in row:  # получение всего содержимого без тегов
#     get_salary(i.text)
...
# ===============================================================================
# 14.06.25 - 15.06.25  серверный сокет и методы
...
#     server_socket.bind(('127.0.0.1', 5000))  # привязывает сокет к адресу
#     server_socket.listen()  # переводит сервер в режим приема соединений
#
#     while True:
#         client_socket, addr = server_socket.accept()  # принимает соединение и блокирует приложение в ожидание сообщения от клиента
#         request = client_socket.recv(1024)  # читает и возвращает данные в двоичном формате из сокета в (макс число байтов в сообщении)
#         print(f"Клиент {addr} => \n{request.decode('utf-8')}\n")
#
#         response = generate_response(request.decode())
#         client_socket.sendall(response)  # отправляет все данные клиенту
...
# cur.execute("INSERT INTO cars VALUES(NULL, 'ZAZ', 1000)")
...
#             .filter(association_table.c.lesson_id == Lesson.id, association_table.c.group_id == Group.id,
...
#     print("*" * 60)
#
#     print(session.query(Lesson).filter(Lesson.lesson_title.in_(['Математика', 'алгебра'])).all())
#     print("*" * 60)
#
#     print(session.query(Lesson).filter(Lesson.lesson_title.notin_(['Математика', 'алгебра'])).all())
#     print("*" * 60)
#
...
#     print("*" * 60)
#
#     for it in (session.query(func.count(Student.surname), Group.group_name).join(Group)
...
# age = 28
#
# tm = Template("Меня зовут {{ name.upper() }}. Мне {{ a*2 }} лет")
